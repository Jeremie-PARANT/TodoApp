...
final class TaskController extends AbstractController
{
    ...
    public function index(Request $request, EntityManagerInterface $entityManager)
    {
        ...
        if ($statusFilter !== null && $statusFilter !== '') {
            $queryBuilder->andWhere('t.status = :status')
                         ->setParameter('status', $statusFilter === '1' ? true : false);
        }
    ...
    #[Route("/task/{id}/update-status", name:"app_task_update_status", methods:["POST"])]
    public function updateStatus(Task $task, EntityManagerInterface $entityManager)
    {
        if ($task) {
            $task->setStatus(true);
